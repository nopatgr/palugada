...
import { ArrowRight, Play, Pause, Monitor, Settings, Wrench, Star, Users, CheckCircle } from "lucide-react"
...
function CountUp({ end, duration = 1200 }: { end: number | string, duration?: number }) {
  ...
  if (typeof end === 'string') return <span>{count}{end.replace(/\d+/g, '')}</span>
          ...
          <div className="space-y-8">
            ...
            <ScrollReveal direction="up" delay={200}>
              <div className="inline-flex items-center px-4 py-2 rounded-full border border-cyan-500/30 bg-cyan-500/10 backdrop-blur-sm">
                ...
                <span className="text-cyan-300 text-sm font-semibold tracking-wide">Hardware Solutions Portfolio</span>
              </div>
            </ScrollReveal>
            ...
            <ScrollReveal direction="up" delay={300}>
              <div className="space-y-6">
                ...
                <p className="text-lg md:text-xl text-slate-200 max-w-lg leading-relaxed tracking-wide">{description}</p>
              </div>
            </ScrollReveal>
            ...
            <ScrollReveal direction="up" delay={500}>
              <div className="flex flex-col sm:flex-row gap-4">
                ...
                <Button
                  variant="outline"
                  size="lg"
                  className="border-white/30 text-white hover:bg-white/10 backdrop-blur-sm transition-all duration-300 transform hover:scale-105 font-semibold tracking-wide"
                  asChild
                >
              ...
              <div className="flex gap-8 mt-8">
                {stats.map((stat, i) => (
                  <div key={i} className="flex flex-col items-center group">
                    <span className={`text-3xl font-bold bg-gradient-to-r ${stat.gradient} bg-clip-text text-transparent group-hover:scale-110 transition-transform duration-300`}>
                      <CountUp end={stat.value} />
                    </span>
                    <span className="text-slate-200 text-sm mt-1 font-medium">{stat.label}</span>
                  </div>
                ))}
              </div>
            </ScrollReveal>
              ...
              <div className="mt-8 p-6 rounded-2xl bg-white/5 backdrop-blur-sm border border-white/10 shadow-lg hover:bg-white/10 transition-all duration-300">
                <div className="flex items-center gap-4">
                  <div className="relative">
                    <img src="/placeholder-user.jpg" alt="Testimoni" className="w-12 h-12 rounded-full object-cover border-2 border-cyan-400" />
                    <div className="absolute -top-1 -right-1 w-4 h-4 bg-cyan-400 rounded-full animate-ping" />
                  </div>
                  <div className="flex-1">
                    <p className="text-sm text-white font-medium leading-relaxed">"Pelayanan cepat, ramah, dan sangat membantu! Tim sangat profesional."</p>
                    <div className="flex items-center gap-2 mt-2">
                      ...
                      <span className="text-xs text-cyan-200 font-medium">- Rina, Customer</span>
                    </div>
                  </div>
                </div>
              </div>
            ...
            <div className="relative">
              ...
              <div className="relative z-10 flex flex-col items-center">
                ...
                <div className="relative mb-6">
                  <Settings className="w-20 h-20 text-blue-400 drop-shadow-xl animate-spin" style={{ animationDuration: '3s' }} />
                ...
                <div className="relative">
                  ...
                  <div className="absolute -top-1 -right-1 w-5 h-5 bg-purple-400 rounded-full animate-bounce" style={{ animationDelay: '0.5s' }} />
                </div>
              </div>

              {/* Floating Elements */}
              <div className="absolute top-10 right-10 w-3 h-3 bg-cyan-400 rounded-full animate-bounce" style={{ animationDelay: '1s' }} />
              <div className="absolute bottom-20 left-10 w-2 h-2 bg-blue-400 rounded-full animate-pulse" style={{ animationDelay: '2s' }} />
              <div className="absolute top-32 left-5 w-4 h-4 bg-purple-400 rounded-full animate-bounce" style={{ animationDelay: '0.5s' }} />
            </div>
          </div>
        </div>
      </div>
